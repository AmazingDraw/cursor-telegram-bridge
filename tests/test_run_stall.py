...
def test_default_stall_timeout_constant() -> None:
    assert DEFAULT_RUN_STALL_TIMEOUT_SEC == 300.0
    assert DEFAULT_RUN_TOOL_STALL_TIMEOUT_SEC == 600.0
...
def test_load_config_health_and_stall(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    (tmp_path / "config.toml").write_text(
        "\n".join(
            [
                ...
                "run_tool_stall_timeout_sec = 480",
    ...
    assert cfg.run_tool_stall_timeout_sec == 480.0
...
def test_load_config_tool_stall_floors_to_thinking(
    ...
    (tmp_path / "config.toml").write_text(
        "\n".join(
            [
                ...
                "run_tool_stall_timeout_sec = 60",
            ]
        ),
        encoding="utf-8",
    )
    cfg = load_config(tmp_path)
    assert cfg.run_stall_timeout_sec == 300.0
    # Tool stall must not be shorter than thinking stall.
    assert cfg.run_tool_stall_timeout_sec == 300.0
...
def test_load_config_rejects_too_small_stall(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert cfg.run_stall_timeout_sec == 300.0
    assert cfg.run_tool_stall_timeout_sec == 600.0
